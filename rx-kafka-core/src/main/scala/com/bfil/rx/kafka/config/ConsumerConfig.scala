...
object ConsumerConfig {
  def apply(config: Config): KafkaConsumerConfig = {
    ...
    val props = new PropertiesBuilder(config).build(Map(
      ...
      "fetch.message.max" -> ("fetch.message.max", ConfigTypes.Bytes),
      ...
      "fetch.min.bytes" -> ("fetch.min.bytes", ConfigTypes.Bytes),
      ...
      "consumer.timeout" -> ("consumer.timeout", ConfigTypes.Duration),
